...
public class MessageProcessorTestSuite {
    ...
    private func testDescribeMessage() async -> TestResult {
        ...
        let success =
            dict["format"]?.string == "json" && dict["namespaces"] != nil && dict["ops"] != nil

        return TestResult(
            name: "testDescribeMessage",
            success: success,
            message: success
                ? "Describe message processed correctly" : "Describe message format incorrect"
        )
    }
    ...
    private func testAddEventMessage() async -> TestResult {
        ...
        let success =
            dict["id"]?.string == "test-id" && checkStatusDone(dict["status"])
        ...
        return TestResult(
            ...
            success: success,
            message: success
                ? "Add event message processed correctly" : "Add event message processing failed"
        )
    }
    ...
    private func testListEventsMessage() async -> TestResult {
        ...
        guard case .dict(let dict) = result else {
            return TestResult(
                name: "testListEventsMessage", success: false, message: "Expected dict")
        }

        let success =
            dict["id"]?.string == "list-id" && checkStatusDone(dict["status"])
        ...
        return TestResult(
            ...
            success: success,
            message: success
                ? "List events message processed correctly"
                : "List events message processing failed"
        )
    }
    ...
    private func testSearchEventsMessage() async -> TestResult {
        ...
        guard case .dict(let dict) = result else {
            return TestResult(
                name: "testSearchEventsMessage", success: false, message: "Expected dict")
        }

        let success =
            dict["id"]?.string == "search-id" && checkStatusDone(dict["status"])
            && dict["value"]?.string != nil
        ...
        return TestResult(
            ...
            success: success,
            message: success
                ? "Search events message processed correctly"
                : "Search events message processing failed"
        )
    }
    ...
    private func testErrorHandling() async -> TestResult {
        ...
        let success =
            dict["id"]?.string == "error-id" && checkStatusError(dict["status"])
