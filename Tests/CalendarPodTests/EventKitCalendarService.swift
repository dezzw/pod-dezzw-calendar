...
public class EventKitCalendarService: CalendarServiceProtocol {
    ...
    public func requestAccess() async throws -> Bool {
        if #available(macOS 14.0, *) {
            // Use the newer API for macOS 14.0+
            ...
            // Fallback to the older API for earlier macOS versions
            return try await withCheckedThrowingContinuation { continuation in
    ...
    public func addEvent(args: InvokeArgs) async throws {
        ...
        guard let startDate = dateFormatter.date(from: args.start),
            let endDate = dateFormatter.date(from: args.end)
        else {
            throw CalendarServiceError.invalidDateFormat
        }

        guard
            let calendar = store.calendars(for: .event).first(where: { $0.title == args.calendar })
        else {
